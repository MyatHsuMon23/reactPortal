...
export default function CardCloseReport() {
  ...
  const { data, isLoading } = useQueryListData({
    queryKey: "fetchCardClosedList",
    url: `${endpoint.getCardClosedReport}`,
    config: {},
    payload: payload
  });

  const { data: BranchList } = useQueryListData({
    queryKey: "fetchBranchListForCardCloseReport",
  ...
  useEffect(() => {
    const filteredRows = data?.data?.Data?.CardCloseInfo?.filter((row: any, index: number) => index >= (listParams.PageIndex - 1) * listParams.PageLimit
          ...
          <Grid item xs={12} sm={12} md={6} lg={3}>
            <Box display={'flex'} justifyContent={'flex-end'}>
              ...
              <Button
                ...
                onClick={() => {
                  ExportToExcel(data?.data?.Data?.CardCloseInfo, `CardCloseReport_${dayjs(filterItem.fromDate).format('YYYY_MMM')}`, columns)
                }}
                color='warning'
                sx={{ margin: '0 10px 0 0' }}
              >
