...
import PrepaidCardIssueForm from './PrepaidCardIssueForm';

const MySwal = withReactContent(Swal);

export default function PrepaidCardIssueList() {
  ...
  const { data, isLoading } = useQueryListData({
    queryKey: "fetchPrepaidIssueList",
    url: `${endpoint.getPrepaidIssueList}`,
    payload: payload,
    config: {},
  });
  ...
  const handleUpdate = async () => {
    ...
    await queryClient.invalidateQueries(['fetchPrepaidIssueList']);
  ...
  return (
    <Box component="div" style={{ width: '100%', marginTop: '10px', position: 'relative' }}>
      ...
      <CustomSwipeableDrawer open={openDrawer} setOpenDrawer={setOpenDrawer}>
        <PrepaidCardIssueForm setOpenDrawer={setOpenDrawer} mode={mode} refetchAPI={() => handleUpdate()} />
