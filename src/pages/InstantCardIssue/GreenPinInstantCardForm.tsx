...
import { CountryList, NewgenderList, SurNameList, docType, regionList } from '@/utils/commonData';
import { FormDatePicker } from '@/components/CustomDatePicker';
import { AddCircle, RestartAlt, Delete, Update, CancelOutlined, Beenhere, TaskAlt, Publish } from '@mui/icons-material';
...
import { AccountNumberValidation, AddressOneValidation, AddressTwoValidation, CleanPhoneNumber, EmailValidation, HouseNoValidation, LuhnCheckValidation, PhoneNoValidation, isFieldRequired, removeTabIndex } from '@/utils/common';
...
const schema = yup
    ...
    .shape({
        ...
        CardNumber: yup.string()
            .required()
            .test('len', val => LuhnCheckValidation(val).message, val => LuhnCheckValidation(val).valid), //9503051027784012
        EmbossName: yup.string().required(),
        buttonType: yup.string()
    })
    .required();
const GreenPinInstantCardForm: React.FC<Props> = ({ setOpenDrawer, mode = 'create', refetchAPI }) => {
    ...
    const onSubmit = (data: any) => {
        ...
        var payload = {
            ...
            CreatedUserId: getItemFromStorage('auth').LoginId, //Add Login User Id
            CreatedUserName: getItemFromStorage('auth').name,
        ...
        if (watch('buttonType') === "delete") {
            url = endpoint.deleteInstantCardIssue;
            var deletePayload = {
            ...
            <form onSubmit={handleSubmit(onSubmit)} action='#'>
                        ...
                        <Grid item xs={4} md={4}>
                            <TextFieldBox
                                ...
                                onSearch={() => {
                                    mutateByAccNo(
                                        {
                                            url: endpoint.getKYCByAccountNo + '?AccountNumber=' + getValues('AccountNumber'),
                                        },
                                        {
                                            onSuccess: (data: any) => {
                                                if (data?.status == 200) {
                                                    ...
                                                    setValue('NrcId', customerData.UidValue != null ? customerData.UidValue.trim() : customerData.UidValue);
                ...
                <Box component="fieldset" sx={{
                    ...
                    <Grid container columnSpacing={2} alignItems={'center'}>
                        <Grid item xs={2} md={2} fontWeight={600} fontSize={14}>
                            <Typography variant='subtitle1' fontSize={14} >Customer Id <span dangerouslySetInnerHTML={{ __html: isFieldRequired(schema, 'CustomerId') }} ></span></Typography>
                        </Grid>
                        <Grid item xs={4} md={4}>
                            <TextFieldBox
                                disabled={true}
...
export default GreenPinInstantCardForm;