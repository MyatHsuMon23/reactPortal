    ...
    const columns: GridColDef[] = [
        {
            ...
            renderCell: (params) => {
                const onClick = (e: any) => {
                    e.stopPropagation();
                    MySwal.fire({
                        html: `Are you sure to activate this card (${maskCard(params?.row?.CARDNUMBER)})?`,
                        icon: "warning",
                        ...
                        if (result.isConfirmed) {
                            mutateAction(
                                {
                                    url: endpoint.creditCardActivate,
                                    body: {
                                ...
                                },
                                {
                                    onSuccess: (data) => {
                                        if (data?.status == 200) {
                                            MySwal.fire({
                                                title: "Card Activation is Successful !",
                                                icon: "success",
                                                confirmButtonColor: theme.palette.primary.main,
    ...
    return (
        ...
        <LoaderWithBackdrop open={isdoingAction} />
            <Typography
                ...
                variant="h5">
                {'Card Activate'} <CheckCircle sx={{ fontSize: '2.5rem' }} />
            </Typography>
